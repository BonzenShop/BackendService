...
public class BackendServiceApplication {
	...
	private static void getConnection() throws ClassNotFoundException, SQLException {
		...
		if(!db.exists()){
			...
			//create tables
			con.createStatement().execute("CREATE TABLE Product(Id int, Name text, Description text, Category varchar(20), Price bigint, OnStock int, Picture int FOREIGN KEY, PRIMARY KEY(Id ASC))");
			...
			con.createStatement().execute("CREATE TABLE ShoppingCartEntry(User int FOREIGN KEY, Product int FOREIGN KEY, Amount int)");
